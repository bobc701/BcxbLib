...
namespace BCX.BCXB {
   ...
   // fielding
      ...
      /// </summary>
      /// -----------------------------------------------------------------
      public void ReadTeam(TextReader f, int ab) { 
      ...
      // px is index into the pitcher matrix, pp.
         ...
         // Line 3: League-level stats...
            ...
            ptr = 1;
            FillLgStats(db_stats, ref lgStats, ref ptr);
         ...
         // Lines 4+: Player records (batter & pitcher)...
            ...
            // Note: important to call ReadTeam(1) first, then (0).
               ...
               b.par.FillBatParas(b.br, lgMean, db_stats[0]);
               ...
               if (db_stats[0] == '2') {
               // It's a Pitcher record, keep reading the same record...
      ...
      private void FillBatStats(string stats, ref CBatRealSet br, ref int ptr) {
         ...
         br.pa = CBCXCommon.GetHex(stats, ref ptr, 3);
         ...
         if (br.ab > 15) {
            ...
            br.ibb = CBCXCommon.GetHex(stats, ref ptr, 2); //--1907.04
         ...
         // 15 or fewer ab's: all stats 1 digit...
            ...
            br.ibb = CBCXCommon.GetHex(stats, ref ptr, 1); //--1907.04
      ...
      // GetHex returns -1 if db_stats is all F's -- this indicates missing.
         ...
         pr.ibb = CBCXCommon.GetHex(stats, ref ptr, 3); //--1907.04
         pr.sv = CBCXCommon.GetHex(stats, ref ptr, 2);
         pr.era = pr.ip3 == 0.0 ? 0.0 : pr.er / ((double)pr.ip3 / 3.0) * 9.0;
      }


      private void FillLgStats(string stats, ref CBatRealSet lgStats, ref int ptr) {
      // ---------------------------------------------------------------------
         lgStats.pa = CBCXCommon.GetHex(stats, ref ptr, 5);
         lgStats.ab = CBCXCommon.GetHex(stats, ref ptr, 5);
         lgStats.h = CBCXCommon.GetHex(stats, ref ptr, 4);
         lgStats.b2 = CBCXCommon.GetHex(stats, ref ptr, 4);
         lgStats.b3 = CBCXCommon.GetHex(stats, ref ptr, 3);
         lgStats.hr = CBCXCommon.GetHex(stats, ref ptr, 3);
         lgStats.so = CBCXCommon.GetHex(stats, ref ptr, 4);
         lgStats.sh = CBCXCommon.GetHex(stats, ref ptr, 3);
         lgStats.sf = CBCXCommon.GetHex(stats, ref ptr, 3);
         lgStats.bb = CBCXCommon.GetHex(stats, ref ptr, 4);
         lgStats.ibb = CBCXCommon.GetHex(stats, ref ptr, 3);
         lgStats.hbp = CBCXCommon.GetHex(stats, ref ptr, 3);
         lgStats.sb = CBCXCommon.GetHex(stats, ref ptr, 3);
         lgStats.cs = CBCXCommon.GetHex(stats, ref ptr, 3);
         //lgStats.ip = (double)(CBCXCommon.GetHex(db_stats, ref ptr, 5)) / 3.0;
         lgStats.ip3 = CBCXCommon.GetHex(stats, ref ptr, 5);
         usesDhDefault = CBCXCommon.GetHex(stats, ref ptr, 1) == 1;
         lgStats.complPct = CBCXCommon.GetHex(stats, ref ptr, 4);
